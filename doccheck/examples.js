 ...
 *
 * Usage:
 *   doccheck examples            typecheck
 *   doccheck examples --keep     keep .doccheck/ for debugging
 */

import fs from 'node:fs';
import path from 'node:path';
import { execFileSync } from 'node:child_process';
import { ROOT, markdownFiles, tscBin } from './files.js';

/**
 * Where the generated examples are compiled.
 *
 * This must be a directory the examples' own imports resolve from, i.e. one with
 * the packages they import installed above it. Compiling somewhere else and
 * bridging with tsconfig `paths` looks equivalent and is not: it breaks React's
 * own types, so `React.ComponentProps<"button">` silently loses className and
 * children and every tsx example is checked against a degraded surface while
 * still reporting a pass. Set `doccheck.workspace` in package.json.
 */
function workDir(config) {
  return path.join(ROOT, config.workspace, '.doccheck');
}
...
function runTsc(WORK) {
  try {
    return execFileSync(tscBin(ROOT), ['-p', 'tsconfig.json'], { cwd: WORK, encoding: 'utf8', stdio: 'pipe' });
  } catch (err) {
...
/**
 * Typecheck every fenced ts/tsx example in the repo's markdown.
 *
 * @param {import("./files.js").DocCheckConfig} config
 * @param {{ keep?: boolean }} [opts]
 * @returns {number} process exit code
 */
export function checkDocExamples(config, opts = {}) {
  const keep = opts.keep ?? false;
  const WORK = workDir(config);

  const docs = markdownFiles(config);
  const blocks = docs.flatMap((d) => extractBlocks(d, fs.readFileSync(path.join(ROOT, d), 'utf8')));
  if (blocks.length === 0) {
    console.error('doccheck examples: no fenced ts/tsx examples found — the fence pattern is wrong.');
    return 1;
  }
  ...
  if (CONFIG_ERROR.test(raw)) {
    console.error('doccheck examples: tsconfig was rejected, so nothing was type checked:\n');
  ...
  if (!diags.some((d) => d.file === CANARY)) {
    console.error('doccheck examples: the canary type error was not reported — semantic checking did not run.\n');
    console.error(raw.trim());
    return 1;
  }
  ...
  if (real.length === 0) {
    ...
    console.log(
      `doccheck examples: ${live.size} example(s) across ${docs.length} markdown file(s) typecheck.`,
    );
  ...
  console.error(`\ndoccheck examples: ${real.length} error(s) in fenced ts/tsx examples:\n`);
