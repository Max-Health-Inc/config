...
import { ROOT, markdownFiles, lineOf } from './files.js';
...
/**
 * Check every badge and in-repo link in the markdown this repo owns.
 *
 * @param {import("./files.js").DocCheckConfig} config
 * @param {{ online?: boolean, json?: boolean }} [opts]
 * @returns {Promise<number>} process exit code
 */
export async function checkBadges(config, opts = {}) {
  const { online = false, json: asJson = false } = opts;
  const pkg = config.pkg;
  const slug = /github\.com\/([^/]+\/[^/.]+)/.exec(pkg.repository?.url ?? '')?.[1];
  if (!slug) {
    console.error('doccheck badges: package.json has no GitHub repository url to validate against.');
    return 1;
  }
  ...
  if (findings.length === 0) {
    console.log(
      `doccheck badges: ${badgeCount} badge(s) across ${files.length} markdown file(s) are consistent.`,
    );
    return 0;
  }

  console.error(`\ndoccheck badges: ${findings.length} problem(s):\n`);
